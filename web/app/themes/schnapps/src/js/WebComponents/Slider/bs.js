...
var BlazeSlider = (function () {
  ...
  function scrollPrev(slider, slideCount) {
    ...
    if (!slider.config.loop) {
      ...
      if (slider.isDragging) {
        ...
        rAf(() => {
          // Force a recalculation of the slider state before reset
          slider.slides = slider.track
            .assignedNodes({ flatten: true })
            .filter((node) => node.nodeType === Node.ELEMENT_NODE);
          slider.states = calculateStates(slider);
          reset();
        });
      }
    }
  ...
  function scrollNext(slider, slideCount) {
    ...
    if (!slider.config.loop) {
      ...
      setTimeout(() => {
        ...
        // Allow time for DOM updates to propagate
        // Update slider state immediately without setTimeout
        slider.slides = slider.track
          .assignedNodes({ flatten: true })
          .filter((node) => node.nodeType === Node.ELEMENT_NODE);
        slider.states = calculateStates(slider);
  ...
  function wrapPrev(slider, count) {
    // Find the custom element host that contains the track slot
    const host = slider.track.getRootNode().host;

    // If no host is found, we can't proceed
    if (!host) {
      console.error("Could not find custom element host for the track slot");
      return;
    }

    // Get all the light DOM children of the host that are element nodes
    const lightDomChildren = Array.from(host.children).filter(
      (node) => node.nodeType === Node.ELEMENT_NODE,
    );
    const len = lightDomChildren.length;

    if (len === 0) return;

    // Move the last 'count' elements to the beginning
    for (let i = 0; i < count; i++) {
      const lastChild = lightDomChildren[len - 1 - i];
      if (lastChild) {
        host.insertBefore(lastChild, host.firstChild);
      }
    }

    // Update the slides reference
    slider.slides = slider.track
      .assignedNodes({ flatten: true })
      .filter((node) => node.nodeType === Node.ELEMENT_NODE);
  }
  function wrapNext(slider, count) {
    ...
    if (lightDomChildren.length === 0) return;

    // Move the first 'count' elements to the end
    for (let i = 0; i < count; i++) {
  ...
  class BlazeSlider extends Automata {
    constructor(blazeSliderEl, blazeConfig) {
      ...
      // Add a slotchange event listener to update slides when slot content changes
      track.addEventListener("slotchange", () => {
        const newSlides = track
          .assignedNodes({ flatten: true })
          .filter((node) => node.nodeType === Node.ELEMENT_NODE);
        // Only refresh if the number of slides has changed
  ...
  function construct(config, slider) {
    ...
    // Ensure the track has a slotchange event listener
    track.removeEventListener("slotchange", handleSlotChange);
    track.addEventListener("slotchange", handleSlotChange);

    function handleSlotChange() {
      const newSlides = track
        .assignedNodes({ flatten: true })
        .filter((node) => node.nodeType === Node.ELEMENT_NODE);
      // Only reconstruct if the number of slides has changed
      if (slider.slides.length !== newSlides.length) {
        slider.slides = newSlides;
        constructAutomata(slider, slider.slides.length, slider.config);
      }
    }
