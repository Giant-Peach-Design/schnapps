...
class Schnapps extends SchnappsFramework
{
  ...
  public function enqueueBaseAssets(): void
  {
    if (defined("WP_DEBUG") && WP_DEBUG) {
      ...
      wp_enqueue_style(
        handle: "schnapps",
        src: get_template_directory_uri() . $this->vite_asset("src/main.css"),
        deps: [],
        ver: null,
      );
      ...
      wp_enqueue_script(
        handle: "schnapps",
        src: get_template_directory_uri() . $this->vite_asset("src/main.js"),
        deps: [],
        ver: null,
        args: true,
      );
  ...
  {
    $asset_file_path = get_parent_theme_file_path(
      "/dist/editor/editor.asset.php",
    );
  ...
  function vite_asset($entry)
  {
    ...
    error_log("Looking for entry: " . $entry);
    error_log("Available entries: " . implode(", ", array_keys($manifest)));

    if (!isset($manifest[$entry])) {
      error_log("Entry '$entry' not found in manifest");
      return "/dist/$entry";
    }

    $file_path = "/dist/" . $manifest[$entry]["file"];
    error_log("Found file: " . $file_path);
    return $file_path;
  }
}
